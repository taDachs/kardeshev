...
std::string NaiveNameGenerator::generateName() const
{
  ...
  int length = util::RandomDistribution::sample(m_min_length, m_max_length + 1);

  bool vowel = util::RandomDistribution::sample(0, 2) == 1;
  int num    = 0;
  for (int i = 0; i < length; i++)
  {
    if (num == 0)
    {
      vowel = !vowel;
      num =
        util::RandomDistribution::sample(1, (vowel ? m_max_num_vowels : m_max_num_consonants) + 1);
    }
    char next_char;
    if (vowel)
    {
      next_char = VOWELS[util::RandomDistribution::sample(0, VOWELS.size())];
    }
    else
    {
      next_char = CONSONANTS[util::RandomDistribution::sample(0, CONSONANTS.size())];
    }
    out += next_char;
    num--;
  }
...
{
  std::string out;
  int length = util::RandomDistribution::sample(m_min_tokens, m_max_tokens);
  for (int i = 0; i < length; i++)
  {
